...
工作时段以请求中的明确设置为准，初始 UI 可填工作日 09:00–18:00，但需可修改并展示。固定事件、已安排任务及本次提案共同扣除容量。任务超过截止、时段重叠、工作时段外和无法排下均返回结构化冲突；禁止 AI 确认带阻断冲突的提案。

### 2.1 时长与拖动（2026-09-16 调整）

时长有三个来源，优先级从高到低：

1. **已安排的起止时间**：日历块的长度就是安排时长（`intervalFor`）。拖到日历或拖块的边缘都会直接决定它，不需要先填数字。
2. **用户设过的 `estimated_minutes`**（可选）：只在任务还没安排时用作估算，并在自动排期时参与容量计算。
3. **默认任务时长 `DEFAULT_TASK_MINUTES`（30 分钟）**：前两者都没有时使用，并在冲突消息里标注为估算。

对标滴答清单的交互：输入靠拖、不靠填。因此下面这些原有的阻断项已降级：

- `UNKNOWN_DURATION`：不再是 blocker。缺估时不再阻止拖到日历，只保留一条 warning，说明重排会按默认 30 分钟估算它。
- 安排长度短于 `estimated_minutes`（同为 `INSUFFICIENT_CAPACITY` 码）：降为 warning。拖出来的长度就是安排结果；如果这里仍然阻断，用户就无法先把大任务安排一段。
- 重排容量计算不再跳过缺估时任务（旧实现在 `capacity.ts` 直接 `return []`），而是按默认时长计入，并在 `UNSCHEDULED_WORK` / `INSUFFICIENT_CAPACITY` 消息里写明“估算”。

原文“未知耗时要求用户补充或列为未解决，**不能悄悄排入**”的约束仍然成立：反对的是“不知不觉”，不是“估算”本身。凡是用了默认值的地方，提案和冲突消息都必须能看出来。仍为 blocker 的是真实冲突：时段重叠、超出工作时段、超过截止、无法放入任何空闲段。

`estimated_minutes` 字段本身保留：已有任务的值不会被清空，模型在 capture 提案里的估时也会保留并显示为只读提示。变化的只是不再让用户手填。
...
今日标记（`PUT /api/items/:id/today`、`DELETE /api/items/:id/today`）是幂等的意图操作：服务端按绝对值 UPSERT/DELETE，不读取其他字段再写回，不存在丢失更新。这里新版 UI 不携带 `expectedVersion`。原因是写入今日标记本身会经 `today_items` 触发器推进 `items.version`，若携带列表里的版本，「加入今日」后紧接着「设为今日重点」必然 409，而这不是并发冲突，是客户端刚制造的版本推进。需要保护用户输入的任务编辑、分类/项目关联、日期安排和提醒写入继续携带版本。
