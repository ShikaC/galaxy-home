...
> 本文描述 `681a222` 功能基线的 AI 工作流与已知限制。新版任务、日历与 AI 重排需求见 [product-requirements.md](product-requirements.md)；当前接口限制不作为新产品的永久边界。
...
本轮使用用户配置的 `gpt-5.6-luna`，在独立 SQLite 工作空间中模拟求职者：生成简历准备计划、确认任务、完成一项任务、记录项目证据、提炼笔记、引用知识并复用未完成任务。体验数据是虚构案例，凭据通过原秘密文件路径读取，没有复制进仓库。

使用中发现并实现：

- 浏览器通过 `Prefer: respond-async` 提交计划，收到 202 和已持久化的运行后立即进入详情；刷新可继续查询，生成时可取消。未带该请求头的调用仍保持同步响应。取消阻止接受结果、自动修正和任务写入；已发出的服务商请求可能继续计费。
- `needs_input` 在问题旁直接接收补充回答，保留原始目标、追问、时间和权限约束。网络重试只在同一来源计划与全部输入相同时复用请求 ID。
- 待确认草稿可调整标题、分钟、日期或移除任务，不调用模型。复用任务保持原标题。保存通过原有结构、引用、日期、预算和重复检查，增加 `proposalRevision`，随后仍需单独确认。
- 编辑器锁定打开时的草稿与版本，后台刷新不会提高它的提交版本。PATCH 使用 `expectedRevision`，确认也绑定版本；旧版本收到 409，已成功执行的重放仍幂等。旧记录没有版本字段时按零处理。
- 服务关闭时先停止接受新的规划请求，再等待已开始的生成，最后关闭数据库。意外进程退出由租约恢复为失败，需重新生成；没有实现跨进程自动续跑。

### 评测报告 v2：正确性与质量提示分开

真实调用揭示 `topicHeuristic` 的误报：任务“选出能够证明案例验证结果的材料”合理，但没有重复“作品集”而被判失败。报告 schemaVersion 2 / metricVersion `hard-checks-with-quality-signals-v2` 将 `topicHeuristic`、`injectionTitleHeuristic` 保留为 `qualitySignals`，并列出 `qualityReviewNeeded`；其余约束检查仍决定 `passed`，任何硬性失败继续使 CLI 非零退出。

这不是模型成功率提升。首轮作品集 5 例旧口径 3/5，等口径重算硬性检查为 5/5；复测同样 5/5，两个词面提示仍为 false。提示词始终为 `workspace-plan-v1`，没有为关键词检查修改提示词。报告保留完整 proposal，供人工评估具体动作、依据和产出。词面提示没有替代人工语义审阅，也不能证明提示注入防护。

具体数据与限制见 [真实模型体验记录](live-model-usage.md)。
