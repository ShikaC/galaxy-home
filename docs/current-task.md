...
- 多提醒使用规则数组；提醒时间变化使旧待触发记录失效，旧 reminderMinutes 保留兼容入口。完全退出后的平台提醒仍需独立实现/实测。
...
仍未验收：原生 OS 通知实际显示与权限弹窗、完全退出后的后台提醒、跨设备同步、移动端、安装升级和持续使用效果；真实模型 8 次请求证明样例流程，不代表总体语义质量。
