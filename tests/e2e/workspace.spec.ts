...
test.describe
  .serial("AI personal workspace", () => {
    ...
    test("merged quick capture, command palette and appearance remain available", async ({
      page,
    }) => {
      await page.keyboard.press("Control+n")
      await page.getByLabel("标题", { exact: true }).fill("主目录合并后的今日行动")
      await page.getByRole("button", { name: "放进今天", exact: true }).click()
      await expect(page.getByText("主目录合并后的今日行动", { exact: true })).toBeVisible()
      await page.keyboard.press("Control+Shift+k")
      await page.getByLabel("搜索空间", { exact: true }).fill("打开设置")
      ...
      await page.reload()
      await expect(page.locator("html")).toHaveAttribute("data-theme", "night")
      await page.getByRole("button", { name: "拂晓", exact: true }).click()
