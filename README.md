...
多提醒可分别绑定时间截止或安排开始，时间修改后旧待提醒失效；延期请求保存首次绝对提醒时间，重试、重启和恢复后不重复向后延期。系统通知由桌面进程自己向服务端领取并投递（`src-tauri/src/notifications.rs`），因此关掉窗口后仍然会弹：窗口只隐藏，进程留在托盘继续运行。托盘菜单的「退出」是真正的退出，退出期间不投递，下次启动按原有逻辑补显。仍未承诺的两件事：OS 实际显示（系统权限、专注模式等由系统裁决），以及完全退出后的定时投递——`tauri-plugin-notification` 在桌面端接受 `Schedule` 但不消费它，那条路需要各平台自写原生代码。
